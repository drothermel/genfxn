...
<Card>
	...
	<CardContent>
		{#if isPiecewise}
			<div class="space-y-4">
				<div>
					<h4 class="mb-2 text-sm font-medium text-muted-foreground">Branches</h4>
					<div class="space-y-2">
						{#each task.spec.branches as branch, i}
							<div class="flex items-center gap-3 rounded-lg border bg-muted p-3">
								<span class="text-sm font-medium text-muted-foreground/70">if</span>
				...
				<div>
					<h4 class="mb-2 text-sm font-medium text-muted-foreground">Default</h4>
					<div class="rounded-lg border bg-muted p-3">
						<span class="text-sm text-muted-foreground/70">else</span>
						<ExpressionView expression={task.spec.default_expr} />
					</div>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="space-y-2 text-sm">
					<div class="flex items-center gap-2">
						...
						<PredicateView predicate={task.spec.predicate} variable="elem" />
					</div>
					<div class="flex items-center gap-2">
						<span class="text-muted-foreground">If true:</span>
						<TransformView transform={task.spec.true_transform} variable="elem" />
					</div>
					<div class="flex items-center gap-2">
						<span class="text-muted-foreground">If false:</span>
						<TransformView transform={task.spec.false_transform} variable="elem" />
					</div>
					<div class="flex items-center gap-2">
						<span class="text-muted-foreground">Init value:</span>
						<code class="rounded bg-muted px-1.5 py-0.5">{task.spec.init_value}</code>
					</div>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="space-y-2 text-sm">
					<div class="flex items-center gap-2">
						...
						<PredicateView predicate={task.spec.reset_predicate} variable="elem" />
					</div>
					<div class="flex items-center gap-2">
						<span class="text-muted-foreground">Init value:</span>
						<code class="rounded bg-muted px-1.5 py-0.5">{task.spec.init_value}</code>
			...
			<div class="space-y-4">
				...
				<div class="space-y-2 text-sm">
					<div class="flex items-center gap-2">
						...
						<PredicateView predicate={task.spec.match_predicate} variable="elem" />
					</div>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="space-y-2 text-sm">
					<div class="flex items-center gap-2">
						...
						<code class="rounded bg-muted px-1.5 py-0.5">{task.spec.tie_break}</code>
					</div>
					<div class="flex items-center gap-2">
						<span class="text-muted-foreground">Empty default:</span>
						<code class="rounded bg-muted px-1.5 py-0.5">{task.spec.empty_default}</code>
					</div>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="space-y-2 text-sm">
					<div class="flex items-center gap-2">
						...
						<code class="rounded bg-muted px-1.5 py-0.5">{task.spec.target}</code>
					</div>
					<div class="flex items-center gap-2">
						<span class="text-muted-foreground">Counting mode:</span>
						<code class="rounded bg-muted px-1.5 py-0.5">{task.spec.counting_mode}</code>
					</div>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="space-y-2 text-sm">
					...
					<div class="flex items-center gap-2">
						...
						<code class="rounded bg-muted px-1.5 py-0.5">{task.spec.invalid_k_default}</code>
					</div>
				</div>
			</div>
		{:else if isStringrules}
			{@const rules = task.spec.rules as Array<{ predicate: Record<string, unknown>; transform: Record<string, unknown> }>}
			{@const defaultTransform = task.spec.default_transform as Record<string, unknown>}
			<div class="space-y-4">
				<div>
					...
					<div class="space-y-2">
						{#each rules as rule, i}
							<div class="flex items-center gap-3 rounded-lg border bg-muted p-3">
								<span class="text-sm font-medium text-muted-foreground/70">{i === 0 ? 'if' : 'elif'}</span>
		...
		{:else}
			<pre class="overflow-x-auto text-xs">{JSON.stringify(task.spec, null, 2)}</pre>
		{/if}
	</CardContent>
</Card>
