...
def generate_stack_bytecode_queries(
    ...
    coverage_inputs = [
        _rand_list(length, _DEFAULT_VALUE_RANGE, rng)
    ...
    patterned = [
        _clamp(base_pattern[i % len(base_pattern)], _DEFAULT_VALUE_RANGE)
        for i in range(adv_len)
    ]
    zeros = [_clamp(0, _DEFAULT_VALUE_RANGE) for _ in range(adv_len)]
    queries.append(
        Query(
            input=patterned,
