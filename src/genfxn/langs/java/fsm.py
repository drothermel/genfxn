...
def _render_state_transitions(spec: FsmSpec) -> list[str]:
    ...
    for idx, state in enumerate(states):
        ...
        state_id = java_int_literal(state.id)
        lines.append(f"            {prefix} (state == {state_id}) {{")
        ...
        for t_idx, transition in enumerate(state.transitions):
            ...
            lines.append(
                "                    state = "
                f"{java_int_literal(transition.target_state_id)};"
            )
...
def _render_accept_expr(spec: FsmSpec) -> str:
    ...
    if not accept_ids:
        return "false"
    if len(accept_ids) == 1:
        return f"state == {java_int_literal(accept_ids[0])}"
    return " || ".join(
        f"state == {java_int_literal(state_id)}" for state_id in accept_ids
    )
...
def render_fsm(spec: FsmSpec, func_name: str = "f", var: str = "xs") -> str:
    ...
    lines = [
        ...
        f"    int state = {java_int_literal(spec.start_state_id)};",
        "    int transitionCount = 0;",
        f"    int sinkStateId = {java_int_literal(sink_state_id)};",
        "",
        f"    for (int x : {var}) {{",
