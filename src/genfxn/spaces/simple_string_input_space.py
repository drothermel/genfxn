...
from genfxn.ops.char_style_transform_op import CharStyleTransformOp
...
def _core_style_space() -> CharStyleTransformSpace:
    return CharStyleTransformSpace()
...
class SimpleStringInputSpace(StringSpace):
    ...
    core_style_space: Space = Field(default_factory=_core_style_space)
    ...
    def validate_sampler_spaces(self) -> SimpleStringInputSpace:
        ...
        if not isinstance(self.core_style_space, Space):
            raise ValueError(
                "core_style_space must implement Space(validate_member, sample)"
            )
    ...
    def sample(self, n_samples: int, rng: random.Random) -> list[str]:
        ...
        for i, core_len in enumerate(lengths):
            ...
            core_styles = cast(
                list[CharStyleTransformType],
                self.core_style_space.sample(core_len, rng),
            )

            core_chars: list[str] = []
            for letter, style in zip(core_letters, core_styles, strict=True):
                core_chars.append(
                    CharStyleTransformOp(transform=style).eval(letter)
                )
