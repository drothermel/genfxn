...
_SCRIPT = (
    ...
    / "scripts"
    / "calibrate_temporal_logic.py"
)

_SCRIPT_MODULE = load_script_module(
    _SCRIPT, "tests.calibrate_temporal_logic_script_module"
)
calibrate_temporal_logic_main = cast(Callable[..., None], _SCRIPT_MODULE.main)
...
def test_main_writes_report_when_strict_checks_pass(
    ...
    def _fake_get_difficulty_axes(
        ...
        assert family == "temporal_logic"
        assert rng is not None
        return _FakeAxes(difficulty)

    def _fake_generate_temporal_logic_task(
    ...
    def _fake_generate_suite(
        ...
        assert seed > 0
        assert pool_size > 0
        return [_FakeSuiteTask() for _ in range(12)]
    ...
    monkeypatch.setattr(
        ...
        _fake_generate_temporal_logic_task,
    )
    monkeypatch.setattr(
        _SCRIPT_MODULE,
        "generate_suite",
    ...
    output = tmp_path / "temporal_logic_calibration.json"
    calibrate_temporal_logic_main(
        output=output,
        samples=5,
        seed=7,
        pool_size=11,
    ...
    assert report["family"] == "temporal_logic"
