...
_SCRIPT = (
    ...
    / "scripts"
    / "calibrate_graph_queries.py"
)

_SCRIPT_MODULE = load_script_module(
    _SCRIPT, "tests.calibrate_graph_queries_script_module"
)
calibrate_graph_queries_main = cast(Callable[..., None], _SCRIPT_MODULE.main)
...
def test_main_writes_report_when_strict_checks_pass(
    ...
    def _fake_get_difficulty_axes(
        ...
        assert family == "graph_queries"
        assert rng is not None
        return _FakeAxes(difficulty)

    def _fake_generate_graph_queries_task(
    ...
    monkeypatch.setattr(
        ...
        _fake_generate_graph_queries_task,
    )
    monkeypatch.setattr(
        _SCRIPT_MODULE,
        "generate_suite",
    ...
    output = tmp_path / "graph_queries_calibration.json"
    calibrate_graph_queries_main(
        output=output,
        samples=5,
        seed=7,
        pool_size=11,
    ...
    assert report["family"] == "graph_queries"
