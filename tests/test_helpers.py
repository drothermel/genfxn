...
def test_require_rust_runtime_fails_when_rustc_not_available(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def _fake_which(name: str) -> str | None:
        if name == "rustc":
    ...
    with pytest.raises(pytest.fail.Exception, match="rustc not available"):
        helpers.require_rust_runtime()


def test_require_rust_runtime_fails_when_version_check_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def _fake_which(name: str) -> str:
        return f"/usr/bin/{name}"
