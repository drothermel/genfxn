#!/usr/bin/env python
"""Calibrate sequence_dp difficulty targeting and suite quota coverage."""
...
app = typer.Typer(help="Calibrate sequence_dp difficulty targeting.")
    ...
    *,
    ...
    for difficulty in DIFFICULTIES:
        ...
        for _ in range(samples):
            ...
            task = generate_sequence_dp_task(axes=axes, rng=rng)
            if task.difficulty is None:
                raise RuntimeError(
                    "sequence_dp task produced None difficulty "
                    "during calibration"
                )
...
def main(
    output: Path = typer.Option(
        Path("artifacts/sequence_dp_calibration.json"),
        "--output",
        "-o",
    ...
    """Run sequence_dp calibration checks and write a JSON report."""
