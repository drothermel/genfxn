#!/usr/bin/env python
"""Calibrate temporal_logic difficulty targeting and suite quota coverage."""
...
app = typer.Typer(help="Calibrate temporal_logic difficulty targeting.")
...
OUTPUT_OPTION = typer.Option(
    Path("artifacts/temporal_logic_calibration.json"),
    "--output",
    "-o",
    ...
    *,
    ...
    for difficulty in DIFFICULTIES:
        ...
        for _ in range(samples):
            ...
            task = generate_temporal_logic_task(axes=axes, rng=rng)
            if task.difficulty is None:
                raise RuntimeError(
                    "temporal_logic task produced None difficulty "
                    "during calibration"
                )
...
def main(
    ...
    """Run temporal_logic calibration checks and write a JSON report."""
    ...
    typer.echo(f"Saved calibration report to {output}")
    for difficulty in DIFFICULTIES:
        stats = reachability[difficulty]
        typer.echo(
            f"  {FAMILY} {_difficulty_label(difficulty)}: "
    ...
    if strict:
        typer.echo(
            "  strict checks: "
            + ("PASSED" if strict_report["passed"] else "FAILED")
        )
        if not strict_report["passed"]:
            for failure in strict_report["failures"]:
