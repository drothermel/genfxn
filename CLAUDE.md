...
must run this exact sequence after making code changes. Update this list after
user approval if any command drifts from the current repo setup.

1. Run `uv run ruff format` on all Python files in the repository.
2. Run `uv run ruff check --fix src/ scripts/`.
